...
export class STOMPService {
  ...
  public configure(config?: StompConfig): void {
    ...
    if (this.state.getValue() !== STOMPState.CLOSED) {
      throw Error('Ya se esta ejecutando!');
    }
    if (config === null && this.config === null) {
      throw Error('No se ha proporcionado la configuracion!');
    }
    ...
    this.client = Stomp.client(`${scheme}://${this.config.host}:${this.config.port}/notaspro-rest/notaspro-app`);
  ...
  public try_connect(): Promise<{}> {
    ...
    if (this.state.getValue() !== STOMPState.CLOSED) {
      throw Error('No se puede intentar try_connect si no esta cerrada la conexion!');
    }
    if (this.client === null) {
      throw Error('Cliente no configurado!');
    }
  ...
  /** Envia el mensaje al topico publish desde el servidor */
  // public publishServer(message?: string): void {
  public publishServer(message?: any): void {
    console.log('Executing publishServer()...');
    this._http.post('http://localhost:8080/notaspro-rest/socket/broadcastMsg', message, null).toPromise().then( (response) => {
      console.log(response);
    });

  }
  public publishStudent(studentData?: any): void {
    console.log('Executing publishStudent()...');
      this._http.post('http://localhost:8080/notaspro-rest/socket/broadcastStudent', studentData, null).toPromise().then( (response) => {
      console.log(response);
    });
  }
  ...
  public on_connect = () => {
    ...
    this.debug('on_connect() > Connectado!');
  ...
  public on_error = (error: string | Stomp.Message) => {
    ...
    if (error.indexOf('Lost connection') !== -1) {
      ...
      this.debug('on_error() > Reconnectando en 5 segundos...');
  ...
  public on_message = (message: Stomp.Message) => {
    ...
    if (message.body) {
      ...
      console.error('on_message() > Mensaje Vacio recibido!');
    }
  }
}
